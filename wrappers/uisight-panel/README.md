...
An alias for the `uisight-panel` command in [uisight](https://www.npmjs.com/package/uisight) --
the live panel you and your agent share.

```bash
npx -y uisight-panel          # this package
npx -y -p uisight uisight-panel   # the same thing, without it
```
...
uisight-panel` always resolves the newest — which is why the MCP registration
