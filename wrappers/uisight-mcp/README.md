...
An alias for the `uisight-mcp` command in [uisight](https://www.npmjs.com/package/uisight) --
the MCP server your agent connects to.

```bash
npx -y uisight-mcp          # this package
npx -y -p uisight uisight-mcp   # the same thing, without it
```
...
uisight-mcp` always resolves the newest — which is why the MCP registration
